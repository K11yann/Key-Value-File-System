...
basic_logic(void)
{
	...
	val_len = sizeof (val);
	if (kvdb_insert(kvdb, KEY, SLEN(KEY), VAL1, SLEN(VAL1)) || // insert必须成功
	    kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len) ||  //insert完查找key
	    (SLEN(VAL1) != val_len) || //长度是否一样
	    memcmp(VAL1, val, val_len) || // 值是否相等
	    (+1 != kvdb_insert(kvdb, KEY, SLEN(KEY), VAL1, SLEN(VAL1))) || //第二次insert要失败
	    kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len) || // 再查找key
	    (SLEN(VAL1) != val_len) || //长度是否一样
	    memcmp(VAL1, val, val_len) || // 值是否相等
	    (1 != kvdb_size(kvdb)) || //size是否为1
	    (0 != kvdb_waste(kvdb))) { // waste是否为0
		kvdb_close(kvdb); //按以上检查顺序不符合就close
		TRACE("software");
		return -1;
	}
